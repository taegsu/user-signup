...
def send(*, db_session: Session, type: str, phone_number: str):
    ...
    if type == MessageType.signup:
        ...
        signup_redis_session.set(name=phone_number, value=value, ex=timedelta(seconds=60 * 3))
    elif type == MessageType.reset:
        user = get_user_by_phone_number(db_session=db_session, phone_number=phone_number)
        if user is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="가입하지 않은 유저 입니다.")
        reset_redis_session.set(name=phone_number, value=value, ex=timedelta(seconds=60 * 3))
    else:
