...
from app.common.database.redis.core import reset_session as reset_session
...
def send(*, type: str, phone_number: int):
    ...
    if type == MessageType.signup:
        signup_redis_session.set(name=phone_number, value=value, keepttl=60 * 3)
    elif type == MessageType.reset:
        reset_session.set(key=phone_number, value=value, keepttl=60 * 3)
    else:
        raise HTTPException(
